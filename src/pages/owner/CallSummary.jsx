...
const CallSummary = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await axiosSecure.get(`/business-owner/call-summary/download/${modalState.data.id}`, {
        responseType: 'blob'
      });
      const url = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `Call_${modalState.type.replace(/\s+/g, '_')}_${modalState.data.id}.pdf`);
                    ...
                    modalState.data.transcript.map((msg, idx) => (
                      <div key={idx} className={`flex items-start gap-4 ${msg.role === 'User' ? 'flex-row-reverse' : ''}`}>
                        <div className="w-10 h-10 rounded-full bg-[#1A2255] flex items-center justify-center shrink-0">
                          {msg.role === 'AI' ? <Bot className="w-5 h-5 text-emerald-400" /> : <User className="w-5 h-5 text-blue-300" />}
                        </div>
                        <div className={`bg-[#1A1A1A] text-gray-200 px-5 py-3.5 rounded-2xl text-[15px] max-w-[80%] ${msg.role === 'User' ? 'rounded-tr-sm' : 'rounded-tl-sm'}`}>
                          {msg.content}
                        </div>
                      </div>
                    ))
