...
const PrinterManagement = () => {
  ...
  const columns = [
    { key: "deviceName", Title: "Device Name", width: "25%", render: (row) => <div className="text-left text-gray-200 font-medium">{row.deviceName || row.device_name || 'N/A'}</div> },
    { key: "serialNumber", Title: "Serial Number", width: "25%", render: (row) => <div className="text-left text-gray-400 font-mono text-sm">{row.serialNumber || row.serial_number || 'N/A'}</div> },
    { key: "status", Title: "Status", width: "15%", render: (row) => (
      <div className="text-left">
        <span className={`inline-block text-center px-3 py-1 text-[11px] font-medium text-white rounded-full capitalize ${row.status?.toLowerCase() === 'online' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/20 text-red-400 border border-red-500/20'}`}>
          {row.status || 'Offline'}
        </span>
      </div>
    )},
    { key: "lastSeen", Title: "Last Seen", width: "20%", render: (row) => <div className="text-left text-gray-400 text-sm">{row.lastSeen ? new Date(row.lastSeen).toLocaleString('en-GB') : 'Never'}</div> },
    {
      key: "action",
      Title: "Action",
      width: "15%",
    ...
    <div>
      ...
      <div className="bg-[#191919] border border-[#1A1A1A] rounded-2xl overflow-hidden shadow-sm">
        {printers.length > 0 ? (
          ...
          <div className="p-12 text-center text-gray-400 flex flex-col items-center justify-center gap-4">
            ...
            <div>
              <h3 className="text-gray-200 text-lg font-medium mb-1">No printers configured</h3>
              <p className="text-sm">Add a printer to start sending orders to your kitchen.</p>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-white">
          <div className="bg-[#111111] border border-[#1A1A1A] rounded-[20px] w-full max-w-[500px] relative shadow-2xl flex flex-col">
            <div className="px-8 py-6 border-b border-[#1A1A1A] flex justify-between items-center">
              <h2 className="text-[17px] font-medium text-gray-200">Add New Printer</h2>
              <button onClick={closeModals} className="text-gray-400 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddSubmit} className="p-8 space-y-5">
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={closeModals} className="px-5 py-2.5 rounded-xl text-sm font-medium text-gray-300 hover:bg-[#1A1A1A] transition-colors">Cancel</button>
                <button type="submit" disabled={addPrinterMutation.isPending} className="flex items-center justify-center bg-[#2563EB] hover:bg-[#1D4ED8] disabled:opacity-70 disabled:cursor-not-allowed transition-colors text-white px-6 py-2.5 rounded-xl text-sm font-medium min-w-[120px]">
                  {addPrinterMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Add Printer"}
                </button>
              </div>
            </form>
          </div>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-white">
          <div className="bg-[#111111] border border-[#1A1A1A] rounded-[20px] w-full max-w-[500px] relative shadow-2xl flex flex-col">
            <div className="px-8 py-6 border-b border-[#1A1A1A] flex justify-between items-center">
              <h2 className="text-[17px] font-medium text-gray-200">Edit Printer Name</h2>
              <button onClick={closeModals} className="text-gray-400 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleEditSubmit} className="p-8 space-y-5">
              ...
              <InputField
                ...
                type="text"
                value={selectedPrinter?.serialNumber || selectedPrinter?.serial_number || ""}
                disabled
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={closeModals} className="px-5 py-2.5 rounded-xl text-sm font-medium text-gray-300 hover:bg-[#1A1A1A] transition-colors">Cancel</button>
                <button type="submit" disabled={editPrinterMutation.isPending} className="flex items-center justify-center bg-[#2563EB] hover:bg-[#1D4ED8] disabled:opacity-70 disabled:cursor-not-allowed transition-colors text-white px-6 py-2.5 rounded-xl text-sm font-medium min-w-[120px]">
                  {editPrinterMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save Changes"}
                </button>
              </div>
            </form>
          </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#0E0E10] border border-gray-800 rounded-[20px] w-full max-w-[450px] p-8 relative shadow-2xl">
            ...
            <h2 className="text-white text-xl font-bold mb-3 text-center">Delete Printer?</h2>
            <p className="text-gray-400 text-[14px] leading-relaxed mb-8 text-center">
              Are you sure you want to delete <span className="text-white font-semibold">{selectedPrinter?.deviceName || selectedPrinter?.device_name}</span>? This printer will no longer receive orders.
            </p>
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              >
                {deletePrinterMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Delete"}
              </button>
            </div>
          </div>
        </div>
