...
const ConnectEmail = () => {
  ...
  const { data: contactResponse, isLoading, refetch } = useQuery({
    queryKey: ['ownerContactInfo'],
    queryFn: async () => {
      const response = await axiosSecure.get('/business-owner/settings/contact-info')
  ...
  const updatePhoneMutation = useMutation({
    mutationFn: async (payload) => {
      const response = await axiosSecure.patch('/business-owner/settings/update-phone', payload)
      return response.data
    },
    onSuccess: (res) => {
      if (res?.success) {
