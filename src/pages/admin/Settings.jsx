...
const Settings = () => {
  ...
  const updateLogoMutation = useMutation({
    mutationFn: async (file) => {
      ...
      const res = await axiosSecure.patch('/system-owner/settings/logo', formData, {
        headers: {
    ...
    <div>
      <div className="bg-[#191919] rounded-2xl p-6 border border-gray-800/50">
        ...
        <div>
          ...
          <label className={`w-25 h-25 rounded-xl border border-gray-600 bg-[#151515] flex items-center justify-center cursor-pointer hover:bg-gray-800/50 transition-colors overflow-hidden relative group ${updateLogoMutation.isPending ? 'opacity-50 pointer-events-none' : ''}`}>
            {isLoading ? (
              <Icon icon="lucide:loader-2" className="text-white text-xl animate-spin" />
            ) : displayLogo ? (
              <>
                <img src={displayLogo} alt="Platform Logo" className="w-full h-full object-cover" />
